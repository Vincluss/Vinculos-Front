...
const Chatbot = () => {
  ...
  const handleBotResponse = (userMessage) => {
    ...
    if (userMessage.toLowerCase().includes("criar currículo")) {
      botMessage =
        "Olá! Eu sou o VINI e estou feliz em te ajudar! Escolha o tipo de seu currículo! Exemplos: freelancer, básico, premium";
    } else if (userMessage.toLowerCase().includes("freelancer")) {
      botMessage =
        "Okay, vamos lá! Eu já tenho algumas informações suas do seu perfil! Digite a sua área de atuação!";
    } else if (userMessage.toLowerCase().includes("add topic")) {
      botMessage =
        "Excellent! What topic do you want to add to the curriculum?";
    } else {
      botMessage =
        "Im sorry, I didnt understand that. Can you please rephrase?";
    }
  ...
  return (
    <div className={Style.chatbotContainer}>
      <div className={Style.chatbotHeader}>
        VINI - Crie comigo seu currículo!
      </div>
      <div className={Style.chatbotMessages}>
        {messages.map((message, index) => (
          <div
            key={index}
            className={message.isUser ? Style.userMessage : Style.botMessage}
          >
            {message.text}
          </div>
        ))}
      ...
      <div className={Style.chatbotInput}>
        ...
        />
        <button onClick={handleSendMessage} className={Style.btn}>
          Enviar
        </button>
      </div>
    </div>
